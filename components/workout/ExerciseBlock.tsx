import { useEffect } from 'react'
import { View, Text, TouchableOpacity } from 'react-native'
import { useWorkoutStore, type WorkoutExercise } from '../../store/workoutStore'
import { SetRow } from './SetRow'

interface ExerciseBlockProps {
  exercise: WorkoutExercise
  lastBest: string | null
  onMount?: () => void
}

export function ExerciseBlock({ exercise, lastBest, onMount }: ExerciseBlockProps) {
  useEffect(() => { onMount?.() }, [])
  const { addSet, copyLastSet } = useWorkoutStore()

  function handleAddSet() {
    const lastSet = exercise.sets[exercise.sets.length - 1]
    addSet(exercise.exerciseId, lastSet?.weightKg ?? 20, lastSet?.reps ?? 5)
  }

  return (
    <View className="bg-card rounded-2xl p-4 mb-4">
      <View className="flex-row justify-between items-center mb-1">
        <Text className="text-accent font-bold text-base">{exercise.exerciseName}</Text>
      </View>
      {lastBest && (
        <Text className="text-muted text-xs mb-3">{lastBest}</Text>
      )}

      {exercise.sets.length > 0 && (
        <View className="flex-row mb-1 px-1">
          <Text className="text-muted text-xs w-6" />
          <Text className="flex-1 text-muted text-xs text-center">kg</Text>
          <Text className="flex-1 text-muted text-xs text-center">reps</Text>
          <View className="w-16" />
        </View>
      )}

      {exercise.sets.map(set => (
        <SetRow key={set.id} set={set} exerciseId={exercise.exerciseId} />
      ))}

      <View className="flex-row gap-2 mt-2">
        <TouchableOpacity
          className="flex-1 bg-card2 rounded-lg py-2 items-center"
          onPress={handleAddSet}
        >
          <Text className="text-white text-sm">+ Add Set</Text>
        </TouchableOpacity>
        {exercise.sets.length > 0 && (
          <TouchableOpacity
            className="flex-1 bg-card2 rounded-lg py-2 items-center"
            onPress={() => copyLastSet(exercise.exerciseId)}
          >
            <Text className="text-muted text-sm">Copy Last</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
  )
}
